...
export async function registerDeviceToken({
	...
	const response = await server.loadJson(
		`${Config.apiUrl}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.REGISTER_TOKEN}`,
		{
			method: 'POST',
			headers: {
...
export async function deleteDeviceToken({secretKey, deviceId, deviceKey}) {
	...
	const requestUrl = `${Endpoints.PROJECT.NOTIFICATIONS.PUSH.DELETE_DEVICE_TOKEN(
		deviceId
...
export async function getDevice({secretKey, idOrKey}) {
	const response = await server.loadJson(
		`${Config.apiUrl}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.GET_DEVICE(
			idOrKey
		)}`,
		{
			method: 'GET',
...
export async function getDevices({
	...
	const requestUrl = `${
		Endpoints.PROJECT.NOTIFICATIONS.PUSH.GET_DEVICES
	}?${toQueryString(request)}`;
...
export async function updateDevice({
	...
	const response = await server.loadJson(
		`${Config.apiUrl}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.UPDATE_DEVICE(
			idOrKey
		)}`,
		{
			method: 'PATCH',
...
export async function getNotifications({
	...
	const requestUrl = `${
		Endpoints.PROJECT.NOTIFICATIONS.PUSH.GET_ALL
	}?${toQueryString(request)}`;
	const response = await server.loadJson(`${Config.apiUrl}${requestUrl}`, {
		method: 'GET',
		headers: {
...
export async function getNotification({secretKey, id, deviceKey}) {
	...
	const requestUrl = `${Endpoints.PROJECT.NOTIFICATIONS.PUSH.GET(
		id
	)}?${toQueryString(request)}`;
	const response = await server.loadJson(`${Config.apiUrl}${requestUrl}`, {
		method: 'GET',
...
export async function sendPushNotification({
	...
	const response = await server.loadJson(
		`${Config.apiUrl}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.SEND}`,
		{
			method: 'POST',
			headers: {
				'X-CM-ProjectId': Config.projectId,
...
export async function markNotificationAsRead({
	...
	const response = await server.loadJson(
		`${
			Config.apiUrl
		}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.MARK_NOTIFICATION_AS_READ(id)}`,
		{
			method: 'PATCH',
			headers: {
				'X-CM-ProjectId': Config.projectId,
...
export async function markNotificationsAsRead({
	...
	const response = await server.loadJson(
		`${Config.apiUrl}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.MARK_NOTIFICATIONS_AS_READ}`,
		{
			method: 'PATCH',
			headers: {
				'X-CM-ProjectId': Config.projectId,
...
export async function getNotificationsCount({
	...
	const requestUrl = `${Config.apiUrl}${
		Endpoints.PROJECT.NOTIFICATIONS.PUSH.GET_NOTIFICATIONS_COUNT
	}?${toQueryString(request)}`;
...
export async function deleteNotification({secretKey, id, deviceKey}) {
	...
	const requestUrl = `${
		Config.apiUrl
	}${Endpoints.PROJECT.NOTIFICATIONS.PUSH.DELETE(id)}?${toQueryString(
		request
	)}`;
	const response = await server.loadJson(requestUrl, {
		method: 'DELETE',
